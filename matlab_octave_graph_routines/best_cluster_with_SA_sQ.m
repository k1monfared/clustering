function [idx, q] = best_cluster_with_SA_sQ(A,max_k)
    % Inputs:
    ...
    [idx_history,q_history] = hierarchical_partition_with_SA_sQ(A,max_k); %find all clustrings of all sizes
    [q,k] = max(q_history); %find the clustring with maximum signed modulairty
    idx = sorted_class_list(idx_history{k}); %the corresponding clustering
end